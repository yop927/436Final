...
class PlaySceneTableViewController: UITableViewController {
    ...
    func startObserving(){
        root.child("games").observe(.value, with: {(snap) in
            var newGames = [MadlibGame]()
            for gameSnaps in snap.children {
                let game = MadlibGame(snapshot: gameSnaps as! DataSnapshot)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "PlayView") as? PlaySceneViewController
        self.navigationController?.pushViewController(vc!, animated: true)
        //vc?.storyContent = games[indexPath.row].text
